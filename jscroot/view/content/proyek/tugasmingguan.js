...
export async function main(){    
    ...
    getJSON(backend.project.assessment,'login',getCookie('login'),getTugasList);
    onClick('tombolmintaapproval', actionfunctionname);
    onChange('tugas-name', handleTugasChange);
    fetchActivityScore();
}

function updateStatusTugas(result) {
    const statusElement = document.getElementById('status-Tugas');
...
function clearStatusTugas() {
    const statusElement = document.getElementById('status-tugas');
    statusElement.textContent = '';
    statusElement.className = '';
}

function handleTugasChange(target) {
    ...
    if (id === defaultValue) {
        ...
        fetchActivityScore();
        // Clear the status TUgas when selecting the default option
        clearStatusTugas();
    } else {
        ...
        getJSON(url, 'login', getCookie('login'), function(result) {
            ...
            // Only update approval status for specific tugas entries
            if (result.status === 200 && result.data.approved !== undefined) {
                updateStatusTugas(result.data.approved);
            }
        });
    }
}

function getTugasList(result) {
