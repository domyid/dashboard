...
function checkAndSubmit() {
    ...
    if (!conditions.isValid) {
        ...
        if (!conditions.qrisCondition) {
            missingItems.push(
                'Minimal salah satu dari QRIS / MBC / RVN harus terisi'
            );
        }
        ...
        Swal.fire({
            icon: 'warning',
            title: 'Belum Lengkap',
            html: `<p>Item berikut masih kurang:</p><ul>${missingItems
                .map((item) => `<li>${item}</li>`)
                .join('')}</ul>`,
...
function checkApprovalButtonConditions() {
    ...
    const { stravakm, iqresult, pomokitsesi, mbc, rupiah, rvn, alltugas } =
        activityData;

    const requiredActivitiesPositive =
        stravakm > 0 && iqresult > 0 && pomokitsesi > 0;
    const qrisCondition = rupiah > 0 || mbc > 0 || rvn > 0;
    const hasTugas =
        Array.isArray(alltugas) &&
        alltugas.some((item) => item.trim && item.trim() !== '');

    // Combine all conditions
    const allConditionsMet =
        requiredActivitiesPositive && qrisCondition && hasTugas;
...
function handleTugasScoreResponse(result) {
    ...
    if (result.status === 200) {
        ...
        updateTableRow(
            3,
            result.data.mbc,
            result.data.mbcPoints || result.data.blockchain
        );
        updateTableRow(
            4,
            result.data.rupiah,
            result.data.qrisPoints || result.data.qris
        );
